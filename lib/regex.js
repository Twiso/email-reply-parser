...
class RegexList {
  constructor() {
    ...
    this.answerHeadersRegex = this.buildRe2([
      ...
      /^>*\s*([а-я]+,\s.+\s.+\n?пише:)$/m,
      /^>*\s*(בתאריך יום ג׳, \d{1,2} באוג׳ \d{4} ב-\d{1,2}:\d{2} מאת .+? <\n?.+?>:)$/m,
      /^>*\s*(Dne\s.+\s.+\n?napsal\(a\):)$/m,
      /^>*\s*(Στις\s.+\s.+\n?έγραψε:)$/m,
      /^>*\s*(tarihinde\s.+\s.+\n?yazdı:)$/m, // tarihinde DATE NAME <EMAIL> yazdı:
      /^>*\s*(La\s.+\s.+\n?a scris:)$/m, // La DATE, NAME <EMAIL> a scris:
      /^>*\s*(Ekkor\s.+\s.+\n?írta:)$/m, // Ekkor DATE NAME <EMAIL> írta:
      /^>*\s*(Vào\s.+\s.+\n?đã viết:)$/m, // Vào DATE, NAME <EMAIL> đã viết:
      /^>*\s*(Pada\s.+\s.+\n?menulis:)$/m, // Pada DATE, NAME <EMAIL> menulis:
      /^>*\s*(เมื่อ\s.+\s.+\n?เขียน:)$/m, // เมื่อ DATE NAME <EMAIL> เขียน:
      /^>*\s*(Dňa\s.+\s.+\n?napísal:)$/m, // Dňa DATE NAME <EMAIL> napísal:
      /^>*\s*(На\s.+\s.+\n?написа:)$/m, // На DATE NAME <EMAIL> написа:
      /^>*\s*(Dana\s.+\s.+\n?napisao:)$/m, // Dana DATE NAME <EMAIL> napisao:
      /^>*\s*(Datums\s.+\s.+\n?rakstīja:)$/m, // Datums DATE NAME <EMAIL> rakstīja:
      /^>*\s*(Kuupäev\s.+\s.+\n?kirjutas:)$/m, // Kuupäev DATE NAME <EMAIL> kirjutas:
      /^>*\s*(Data\s.+\s.+\n?rašė:)$/m, // Data DATE NAME <EMAIL> rašė:
      /^>*\s*(Dne\s.+\s.+\n?je napisal:)$/m, // Dne DATE NAME <EMAIL> je napisal:
      /^>*\s*(Më\s.+\s.+\n?shkroi:)$/m, // Më DATE NAME <EMAIL> shkroi:
    ]);
