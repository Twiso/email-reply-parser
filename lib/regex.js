...
class RegexList {
  constructor() {
    ...
    this.answerHeadersRegex = this.buildRe2([
      ...
      /^>*\s*(\d{1,2}\s[а-яі]+\.\s\d{4}\sр\.\sо\s\d{1,2}:\d{2}\s.+\s<.+>\sпише:)$/m,
      /^>*\s*(בתאריך יום\s[א-ת]׳,\s*\d{1,2}\sב[א-ת]+׳?\s*\d{4}\sב-\d{1,2}:\d{2}\sמאת\s.+:)$/m,
      /^>*\s*(\d{1,2}\s[а-я]+\.?\s\d{4}\s(?:г\.|года)\s(?:в\s)?\d{1,2}:\d{2},\s.+\s<.+>:)$/m
    ]);
